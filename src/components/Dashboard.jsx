...
export default function Dashboard() {
  ...
  const handleSearch = (val) => {
    let newCodeList = codeList.filter((f) =>
  ...
  const changeName = (id, title, code, event) => {
    let newName =
      event.target.parentNode.parentNode.querySelector(".disName").innerText;
      ...
      if (user) {
        const fetchData = async () => {
          try {
            ...
            if (fetchedData.length < 1) return;
            setCodeList(fetchedData);
            setSelectedCode(fetchedData[0]);
            setFilteredCodeList(fetchedData);
          } catch (error) {
  ...
  if (user && !user.emailVerified)
    return (
      <div
        ...
        style={{ padding: "0 10%", fontSize: "0.7em" }}
      >
        <div>
          Hii, {user.displayName} please verify your email by clicking the link
          in your inbox to complete the registration process. Thank you for
          choosing us.
        </div>
      </div>
    );
  ...
  return (
    <div className="mainCont" onKeyDown={(e) => handleKeyPress(e)}>
        ...
        <div className="snippets">
          {filterdCodeList.map((e) => {
            return (
              <div
                className={
                  e.id == selectedCode.id ? "snipCont selected" : "snipCont"
                }
              >
                <div
                  className="snip"
                  id={e.id}
                  onClick={() => setSelectedCode(e)}
                >
                  <div>
                  ...
                  <div
                    className="disName"
                    contentEditable={e.id == selectedCode.id && editEnable}
                  >
                    {e.title.slice(0, 40)}
                  </div>
                </div>
                {e.id == selectedCode.id && editEnable ? (
                  <div>
                    <i
                      className="fa-solid fa-floppy-disk"
                      onClick={(event) =>
                        changeName(e.id, e.title, e.code, event)
                      }
                    ></i>{" "}
                  </div>
                ) : (
                  <div>
                    <i
                      ...
                      onClick={() => handleNameEdit(e.id, e.title, e.code)}
                    ></i>
                  </div>
                )}
      ...
      <div className="codeEditorCont">
        ...
        ></i>
        <i
          class="fa-solid fa-floppy-disk updateBtn"
          onClick={() => updateCode(selectedCode.id)}
        ></i>
        ...
        <i
          ...
          className="fa-solid fa-circle-plus addModuleBtn"
          onClick={() => setAddModuleDis(!addModuleDis)}
        ></i>
      ...
      <div
        className="addCodeCont"
        style={addModuleDis ? { width: "75%" } : { width: "0%" }}
      >
      ...
      <div
        className="prompt"
        style={delPopUp ? { transform: "scale(1)" } : { transform: "scale(0)" }}
      >
        <div className="card">
          <div className="text">Delete {selectedCode.title} ?</div>
          <div className="buttonCont">
            {" "}
            <button onClick={() => handleDelete(selectedCode.id)}>YES</button>
            <button onClick={() => setDelPopUp(false)}>NO</button>
          </div>
        </div>
      </div>
    </div>
